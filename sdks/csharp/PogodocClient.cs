...
{
    public PogodocClient(string token, string? baseUrl) : base(token, new ClientOptions() { BaseUrl = baseUrl })
    {
    }
    ...
    public async Task<string> SaveTemplateFromFileStreamAsync(Stream payload, long payloadLength, SaveTemplateMetadata metadata)
    {
        var initResponse = await PogodocClient.Templates.InitializeTemplateCreationAsync();

        await S3Utils.UploadToS3WithUrlAsync(initResponse.PresignedTemplateUploadUrl, payload, payloadLength, "application/zip");

        await Templates.ExtractTemplateFilesAsync(initResponse.JobId);

        var previewsResponse = await Templates.GenerateTemplatePreviewsAsync(initResponse.JobId, new GenerateTemplatePreviewsRequest
        {
            Type = metadata.Type,
            Data = metadata.SampleData
        });

        await Templates.SaveCreatedTemplateAsync(initResponse.JobId, new SaveCreatedTemplateRequest
        {
    ...
    public async Task<string> UpdateTemplateFromFileStreamAsync(Stream payload, long payloadLength, UpdateTemplateProps metadata)
    {
        ...
        await Templates.UpdateTemplateAsync(metadata.TemplateId, new UpdateTemplateRequest // Pass the original templateId here
        {
    ...
    public async Task<GetJobStatusResponse> GenerateDocumentAsync(GenerateDocumentProps props)
    {
        ...
        if (!string.IsNullOrEmpty(props.Template) && !string.IsNullOrEmpty(initResponse.PresignedTemplateUploadUrl))
        {
        ...
        await Documents.StartRenderJobAsync(initResponse.JobId, new StartRenderJobRequest
        {
